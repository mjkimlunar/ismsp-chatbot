...
export function buildPrompt(question: string, hits: Retrieved[]): string {
  ...
  return [
    ...
    `질문: ${question}`,
    "",
    "위 자료 조각에서 질문과 관련된 내용을 찾아 한국어로 답하세요. 질문의 정확한 단어가 없더라도 내용이 관련되면 그 조각을 근거로 답하세요. [ID]를 표시하고, 숫자·기간·조건은 원문 그대로 인용하세요. 자료 조각 어디에도 관련 내용이 전혀 없을 때만 '자료에 없습니다'라고 답하세요. 추측하지 마세요.",
  ].join("\n");
}
