...
export function buildPrompt(question: string, hits: Retrieved[]): string {
  ...
  return [
    `질문: ${question}`,
    "",
    "아래 자료에서 위 질문의 답을 찾아 답변하세요.",
    "",
    "--- 자료 ---",
    context,
    "--- 자료 끝 ---",
    "",
    "예시) 질문: 갱신은 언제 신청해야 하나?",
    "답변: [AR-28] 인증서 유효기간 만료 3개월 전에 갱신심사를 신청하여야 합니다.",
    "",
    "위 예시처럼 자료의 [ID]와 원문 내용을 인용하여 한국어로 답하세요. 숫자·기간·조건은 원문 그대로 쓰세요. 자료에 전혀 관련 내용이 없을 때만 '자료에 없습니다'라고 답하세요. 자료에 없는 기관명·법률명·수치를 지어내지 마세요. 자료 원문에 나온 내용만 답하세요.",
  ].join("\n");
}
